...
return require('packer').startup{function(use, packer_plugins)
    ...
    config=function()
        require('org-call').setup({pre_commands={
  ...
  wants={"vim-gutentags"},
  requires={{'nvim-lua/plenary.nvim',module='plenary'},{'nvim-telescope/telescope.nvim',module='telescope'}, {"ludovicchabant/vim-gutentags",opt=true}},
  }
  ...
  use {"ludovicchabant/vim-gutentags",
        setup=function() require'plugin_config/gutentags' end,
  ...
  setup = function()
      LazyGitOpener = function()
        if vim.api.nvim_command_output("FloatermShow --name='lazygit' --title='LazyGit'")~='' then
            vim.api.nvim_command("FloatermNew --name='lazygit' --title='LazyGit' --width=0.95 --height=0.95 --autoclose=1 lazygit")
        end
      end
      require('which-key').register(
      {
  ...
  ft= {"tex"}
   ...
   config = function() require'plugin_config/grammalecte' end}
   use {"dhruvasagar/vim-table-mode",
   ft = {"markdown", "pandoc", "tex"},
   config= function()
       require('which-key').register(
       {t = {name='table mode'}},{prefix="<leader>"}
       )
   end
  }
  ...
  use {'heterophyllus/vscode-codev',
  ft={"codev"}
  }

  -- Programming
  use {'GuillaumeAllain/vim-codevmacro',
  branch='dev',
  ft={"codev"}
  }
  ...
  --             g={
  --                 name='plugins',
  --                 t= {':TigStatus<CR>','tig',noremap=true,mode='n'},
  --             }
  --         },
  --         {prefix='<leader>'}
  --         )
  ...
  requires={
   ...
   requires = {
       ...
       config = function() require'plugin_config/treesitter'.setup(require'plugin_config.ft'.treesitter_ft) end
   }
