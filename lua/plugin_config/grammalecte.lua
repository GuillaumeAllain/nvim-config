vim.g.grammalecte_cli_py = vim.fn.expand(os.getenv("HOME") .. "/srv/grammalecte/grammalecte-cli.py")

vim.g.grammalecte_disable_rules = "typo_guillemets_typographiques_doubles_ouvrants "
    .. "esp_fin_ligne "
    .. "esp_début_ligne "
    .. "esp_milieu_ligne "
    .. "typo_tiret_début_ligne "
    .. "apostrophe_typographique "
    .. "apostrophe_typographique_après_t "
    .. "espaces_début_ligne espaces_milieu_ligne "
    .. "espaces_fin_de_ligne "
    .. "typo_espace_manquant_après1 "
    .. "typo_espace_manquant_après2 "
    .. "typo_points_suspension1 "
    .. "typo_points_suspension2 "
    .. "typo_points_suspension3 "
    .. "typo_tiret_incise "
    .. "typo_tiret_incise2 "
    .. "typo_guillemets_typographiques_doubles_fermants "
    .. "nbsp_avant_double_ponctuation "
    .. "nbsp_avant_deux_points "
    .. "nbsp_après_chevrons_ouvrants "
    .. "nbsp_avant_chevrons_fermants1 "
    .. "unit_nbsp_avant_unités1 "
    .. "unit_nbsp_avant_unités2 "
    .. "unit_nbsp_avant_unités3 "
